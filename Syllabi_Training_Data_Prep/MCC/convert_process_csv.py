...
CERTIFIED_MARKERS = {'DATE SUBMITTED', 'CATALOG NO.', 'DATE DICC APPROVED', 'DATE LAST REVIEWED',
                     'COURSE INFORMATION FORM', 'DISCIPLINE', 'COURSE TITLE', 'CR.HR', 'LECT HR.',
                     'LAB HR.', 'CLIN/INTERN HR.', 'CLOCK HR.', 'CATALOG DESCRIPTION',
                     'PREREQUISITES', 'EXPECTED STUDENT OUTCOMES IN THE COURSE',
                     'GENERAL EDUCATION OUTCOMES', 'PROGRAM-LEVEL OUTCOMES',
                     'CAREER AND TECHNICAL EDUCATION PROGRAM OUTCOMES',
                     'CLASS-LEVEL ASSESSMENT MEASURES', 'COURSE OUTLINE FORM',
                     'PROGRAM-LEVEL OUTCOMES ADDRESSED', 'COURSE INFORMATION', 'COURSE OUTLINE',
                     'DATE DICC', 'DICC APPROVAL', 'NO.', 'PROGRAM OUTCOMES', 'ASSESSMENT MEASURES'}
...
PROCESS_CNT = 0
RESULTS = pd.DataFrame(columns=['File Name', 'DATE SUBMITTED', 'CATALOG NO.', 'DATE DICC APPROVED',
                                'DATE LAST REVIEWED', 'DISCIPLINE', 'COURSE TITLE', 'CR.HR.',
                                'LECT.HR.', 'LAB.HR.', 'CLIN/INTERN.HR.', 'CLOCK.HR.',
                                'CATALOG DESCRIPTION', 'PREREQUISITES',
                                'EXPECTED STUDENT OUTCOMES IN THE COURSE',
                                'GENERAL EDUCATION OUTCOMES',
                                'CAREER AND TECHNICAL EDUCATION PROGRAM OUTCOMES',
                                'CLASS-LEVEL ASSESSMENT MEASURES',
                                'PROGRAM-LEVEL OUTCOMES ADDRESSED', 'COURSE OUTLINE FORM',
                                'UPDATE AT'])
...
for i in range(len(DF['file_fullname'])):
    ...
    if isinstance(buff, str):
        ...
        for j in range(len(markers) -1):
            ...
            # In the last part, some duplicate markers need not to be treated as markers
            ...
            if result:
                ...
                elif (markers[j] == 'DATE DICC APPROVED' or markers[j] == 'DATE DICC'
                      or markers[j] == 'DICC APPROVAL'):
                ...
                elif (markers[j] == 'EXPECTED STUDENT OUTCOMES IN THE COURSE'
                      or markers[j] == 'IN THE COURSE'):
                ...
                elif (markers[j] == 'CAREER AND TECHNICAL EDUCATION PROGRAM OUTCOMES'
                      or markers[j] == 'PROGRAM OUTCOMES'):
                    CAREER_AND_TECHNICAL_EDUCATION_PROGRAM_OUTCOMES = result[0].strip()
                elif markers[j] == ('CLASS-LEVEL ASSESSMENT MEASURES'
                                    or markers[j] == 'ASSESSMENT MEASURES'):
        ...
        RESULTS.loc[PROCESS_CNT] = [File_Name, DATE_SUBMITTED, CATALOG_NO, DATE_DICC_APPROVED,
                                    DATE_LAST_REVIEWED, DISCIPLINE, COURSE_TITLE, CR_HR, LECT_HR,
                                    LAB_HR, CLIN_or_INTERN_HR, CLOCK_HR, CATALOG_DESCRIPTION,
                                    PREREQUISITES, EXPECTED_STUDENT_OUTCOMES_IN_THE_COURSE,
                                    GENERAL_EDUCATION_OUTCOMES,
                                    CAREER_AND_TECHNICAL_EDUCATION_PROGRAM_OUTCOMES,
                                    CLASS_LEVEL_ASSESSMENT_MEASURES,
                                    PROGRAM_LEVEL_OUTCOMES_ADDRESSED, COURSE_OUTLINE_FORM,
                                    UPDATE_AT]
